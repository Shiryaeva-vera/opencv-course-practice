...
void drawKpt(Mat& img, const KeyPoint& p, const Scalar& color, int flags, Point offset = Point(0, 0))
{
  ...
  if (flags & DrawMatchesFlags::DRAW_RICH_KEYPOINTS)
  {
	...
	circle(img, center, radius, color, 1, cv::LINE_AA);
    ...
    if (p.angle != -1)
    {
      ...
      line(img, center, center + orient, color, 1, cv::LINE_AA);
    }
  }
  else
  {
...
int main(int argc, char* argv[])
{
  ...
  {
	  KeyPoint objPoint = keypoints1[matches[matchInd].queryIdx];
	  KeyPoint scenePoint = keypoints2[matches[matchInd].trainIdx];
	  
	  Point2f dif = (Point2f)center - objPoint.pt;
	  float dist = (float)norm(dif);
	  float angle = atan(dif.y/dif.x);
	  
	  float delta_angle = (scenePoint.angle - objPoint.angle) * (float)CV_PI/180.f;
	  float delta_scale = scenePoint.size/objPoint.size;

	  float dist2 = dist * delta_scale;
	  Point center = Point(cvRound(scenePoint.pt.x + cos(angle + delta_angle) * dist2), cvRound(scenePoint.pt.y + sin(angle + delta_angle) * dist2));

	  hgt.at<int>(center.y/splitSet, center.x/splitSet) += 1;
  }
  Point maxP;
  ...
  //homography
  vector<Point> matchedKeypoints1(matches.size());
  vector<Point> matchedKeypoints2(matches.size());
  for (size_t matchInd = 0; matchInd < matches.size(); matchInd++)
  {
	  matchedKeypoints1[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
	  matchedKeypoints2[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
  }
  Mat homography = findHomography(matchedKeypoints1, matchedKeypoints2, RANSAC);
